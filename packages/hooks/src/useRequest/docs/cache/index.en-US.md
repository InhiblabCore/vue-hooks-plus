...
| --- | --- | --- | --- |
| cacheKey | A unique ID of the request. When a function is provided, the real cache key is resolved from current params before each request. | `string \| ((params?: TParams) => string \| undefined \| null)` | - |
...
| setCache | <ul><li> Custom set cache, supporting both sync and async cache backends </li><li> `setCache` and `getCache` need to be used together</li><li> In the custom cache mode, `cacheTime` and `clearCache` are useless, please implement it yourself according to the actual situation.</li></ul> | `(data: CachedData) => void \| Promise<void>` | - |
| getCache | Custom get cache, supporting async reads such as `localforage` or IndexedDB | `(params: TParams) => CachedData \| undefined \| Promise<CachedData \| undefined>` | - |
